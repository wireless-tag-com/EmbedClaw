...
| Chat Channel | Feishu、WebSocket | 飞书长连接入站/出站，本地 WebSocket 对话 |
...
│   ├── tools/                    # Tools 实现
│   ├── channel/                  # Feishu / WebSocket 通道实现
│   ├── embed_claw.c              # 系统统一启动入口
│   └── ec_config.h               # 当前项目的编译期配置
├── spiffs_data/                  # 默认写入 SPIFFS 的系统文件
└── scripts/                      # WebSocket 测试与飞书中继脚本
```
...
4. 在出站时根据 `msg->chat_id` 把消息发回对应目标
5. 在 `components/embed_claw/channel/ec_channel_reg.inc` 中增加 `EC_CHANNEL_REG(xxx)`
